...
def odom_callback(odom: Odometry):
    ...
    if odom.pose.pose.position.x > -0.08 and odom.pose.pose.position.x < 0.1 and C1 == False:
        rospy.loginfo("START.")
        cmd.linear.x = 0.1
        cmd.angular.z = 0.0

    elif odom.pose.pose.position.x > -0.1 and odom.pose.pose.position.x < -0.07 and z_degree < -90 and C2 == False:
    ...
    elif odom.pose.pose.position.y > -0.33 and odom.pose.pose.position.y < 0.1 and C3 == True and C4 == False:
        rospy.loginfo("Third condition achieved.")
        cmd.linear.x = 0.1
        cmd.angular.z = 0.0

    elif odom.pose.pose.position.y > -0.35 and z_degree < 0 and C5 == False:
    ...
    elif odom.pose.pose.position.x > -0.73 and C6 == True and C7 == False:
        rospy.loginfo("Sixth condition achieved.")
        if odom.pose.pose.position.x > -0.53 and odom.pose.pose.position.x < -0.08 and forward_speed > -0.18:
            forward_speed = forward_speed -0.005
            print('current speed', forward_speed)
        elif odom.pose.pose.position.x < -0.53:
            forward_speed = -0.1
        cmd.linear.x = forward_speed        
        # cmd.linear.x = -0.1
        cmd.angular.z = 0.0

    elif odom.pose.pose.position.x < -0.73 and C7 == False:
        rospy.loginfo("Seventh condition achieved.")
        cmd.linear.x = 0.0
        cmd.angular.z = 0.0
        C7 = True

    elif odom.pose.pose.position.x < -0.73 and C7 == True and C10 == False:
    ...
    elif odom.pose.pose.position.x < -0.08 and C10 == True and C12 == False:
        rospy.loginfo("Ninth condition achieved.")
        cmd.linear.x = 0.18
        cmd.angular.z = 0.0
        C11 = True

    elif odom.pose.pose.position.x > -0.08 and C11 == True and C12 == False:
    ...
    elif odom.pose.pose.position.y < 0.0 and C15 == True and C17 == False:
        rospy.loginfo("Thirteenth condition achieved.")
        cmd.linear.x = -0.05
        cmd.angular.z = 0.0
        C16 = True

    elif odom.pose.pose.position.y > -0.03 and z_degree > -179 and C16 == True and C18 == False:
